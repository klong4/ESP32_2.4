...
#include "../FT5x26/ft6236.h"
#include <Arduino.h>

/* Display buffer */
#define DISP_BUF_SIZE (DISPLAY_WIDTH * 40)  // 40 lines buffer
...
static void touchpad_read(lv_indev_t *indev_drv, lv_indev_data_t *data)
{
    ...
    if (ft5x26_read_touch(&touch_data) && touch_data.touch_count > 0) {
        // Direct mapping - no transformation needed for parallel ST7789
        uint16_t raw_x = touch_data.points[0].x;
        uint16_t raw_y = touch_data.points[0].y;
        
        // Simple calibration (adjust if needed based on actual touch response)
        int32_t x = raw_x;
        int32_t y = raw_y;
